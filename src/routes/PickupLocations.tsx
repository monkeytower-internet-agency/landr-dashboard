...
export function PickupLocations() {
  ...
  const locationsQuery = useRealtimeQuery<Location[]>({
    queryKey: ['locations', currentOperatorId ?? 'none'],
    queryFn: () => fetchLocations(currentOperatorId as string),
  ...
  const roleTypesQuery = useQuery<LocationRoleType[]>({
    queryKey: ['location-role-types', currentOperatorId ?? 'none'],
    queryFn: () => fetchLocationRoleTypes(currentOperatorId as string),
    enabled: !!currentOperatorId,
  })
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      ) : locationsQuery.isPending && currentOperatorId ? (
        <p className="text-muted-foreground text-sm">
