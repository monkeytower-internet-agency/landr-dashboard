// landr-pugm — per-user persistence of the products list sort mode.
...
describe('useProductsSort', () => {
  it('defaults to Recently added when no preference is stored', () => {
    const { result } = renderHook(() => useProductsSort())
    expect(result.current.sort).toBe(DEFAULT_PRODUCTS_SORT)
  ...
  it('isolates state between users on the same browser', () => {
    const first = renderHook(() => useProductsSort())
    act(() => first.result.current.setSort('name_asc'))

    authState.userId = 'user-2'
    const second = renderHook(() => useProductsSort())
    // user-2 sees the default, not user-1's selection
    expect(second.result.current.sort).toBe(DEFAULT_PRODUCTS_SORT)
